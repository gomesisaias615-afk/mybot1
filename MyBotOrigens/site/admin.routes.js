...
router.post("/api/painel/catalogo/item", exigirAutenticacao, (req,res)=>{try{
  ...
  if(tipo==="pizza"){
    ...
    if(pre[nome])throw Error("Já existe uma pizza com esse nome.");pre[nome]=tamanhos;conf.pizzasPorCategoria[categoria]||=[];conf.pizzasPorCategoria[categoria].push(nome);const chaveEstoque=nome.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[_-]+/g," ").replace(/\s+/g," ").trim();est.pizzas[chaveEstoque]=1000;salvar(p,pre);salvar(c,conf);salvar(e,est);precos.atualizarIngredientesPizza(nome,ingredientes)
  }else if(tipo==="bebida"){
    ...
    if(pre[k])throw Error("Já existe uma bebida com esse nome.");pre[k]=preco;nom[k]={nome,aliases:[k.replaceAll("_"," ")]};est.bebidas[k]=1000;salvar(p,pre);salvar(n,nom);salvar(e,est)
