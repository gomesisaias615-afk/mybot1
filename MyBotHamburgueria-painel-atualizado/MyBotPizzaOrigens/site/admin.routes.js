...
// Os acessos ficam persistentes no dispositivo. O Chrome limita cookies
// persistentes a 400 dias. Alterar um token no Render invalida as sessões
// assinadas com aquele token.
...
function criarSessao(res, perfil = "administrador") {
  ...
  // painel e continua válido por dispositivo em qualquer instância.
...
router.get(["/instalar", "/instalar/"], (req, res) => {
  ...
  const scriptInstalacao = `<script>
    ...
    addEventListener('appinstalled',()=>{promptMyBot=null;botaoMyBot.disabled=true;botaoMyBot.textContent='✓ MYBOT INSTALADO';if(ajudaMyBot)ajudaMyBot.textContent='Pronto! Abra o ícone MyBot para acessar a Central.';});
...
// Portais internos do aplicativo; cardápio e pagamento ficam fora do escopo.
