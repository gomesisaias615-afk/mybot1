...
function renderIngredientes(){const d=$("#listaIngredientes");if(!d)return;const busca=String($("#buscarIngredientes")?.value||"").toLocaleLowerCase("pt-BR").trim();const itens=(estado.ingredientesPizzas||[]).filter(x=>normalizarBuscaPainel(nomePizzaPainel(x.nome)+" hambúrguer hambúrgueres").includes(normalizarBuscaPainel(busca)));d.innerHTML=itens.length?itens.map(x=>'<article class="linha-preco ingrediente-item"><strong>'+escapar(nomePizzaPainel(x.nome))+'</strong><label class="ingredientes-campo">Ingredientes<textarea rows="3" maxlength="500" data-ingredientes-pizza="'+encodeURIComponent(x.nome)+'">'+escapar(x.ingredientes||"")+'</textarea></label><button class="btn primario" data-salvar-ingredientes="'+encodeURIComponent(x.nome)+'">Salvar</button></article>').join(""):'<div class="vazio">Nenhum hambúrguer encontrado.</div>'}
estado.tipoImagem="todos";
function renderImagens(){const d=$("#listaImagens");if(!d)return;const busca=String($("#buscarImagens")?.value||"").toLocaleLowerCase("pt-BR").trim();const itens=(estado.imagensProdutos||[]).filter(x=>{const nomeExibido=x.tipo==="pizzas"?nomePizzaPainel(x.nome):x.nome;const termos=nomeExibido+" "+x.nome+" "+(x.tipo==="pizzas"?"hambúrguer hambúrgueres":"bebida bebidas");return(estado.tipoImagem==="todos"||x.tipo===estado.tipoImagem)&&normalizarBuscaPainel(termos).includes(normalizarBuscaPainel(busca))});d.innerHTML=itens.length?itens.map(x=>{const id=encodeURIComponent(JSON.stringify({tipo:x.tipo,chave:x.chave,nome:x.nome}));return '<article class="imagem-item"><div class="imagem-preview '+(x.imagem?"tem-imagem":"")+'">'+(x.imagem?'<img src="'+escapar(x.imagem)+'" alt="'+escapar(x.nome)+'">':'<span>'+(x.tipo==="bebidas"?"🥤":"🍔")+'</span><small>Sem foto</small>')+'</div><div class="imagem-info"><strong>'+escapar(x.tipo==="pizzas"?nomePizzaPainel(x.nome):x.nome)+'</strong><small>'+(x.tipo==="pizzas"?"Hambúrguer":"Bebida")+'</small><input type="file" accept="image/jpeg,image/png,image/webp" data-arquivo-imagem="'+id+'"><div class="imagem-acoes"><button class="btn primario" data-publicar-imagem="'+id+'">Publicar imagem</button>'+(x.imagem?'<button class="btn perigo" data-remover-imagem="'+id+'">Remover</button>':'')+'</div></div></article>'}).join(""):'<div class="vazio">Nenhum produto encontrado.</div>'}
