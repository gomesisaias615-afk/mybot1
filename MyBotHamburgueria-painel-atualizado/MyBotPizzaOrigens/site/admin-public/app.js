...
async function carregar() {
  ...
  estado.adicionais = await api("/api/painel/adicionais");
...
function renderIngredientes(){const d=$("#listaIngredientes");if(!d)return;const busca=String($("#buscarIngredientes")?.value||"").toLocaleLowerCase("pt-BR").trim(),tipo=estado.tipoDescricao||"todos",rotulos={tradicionais:"hamburgueres",especiais:"acompanhamentos",doces:"combos"};const itens=(estado.ingredientesPizzas||[]).filter(x=>{const tipoItem=rotulos[x.categoria]||"hamburgueres",texto=`${nomePizzaPainel(x.nome)} ${x.nome} ${tipoItem}`;return(tipo==="todos"||tipo===tipoItem)&&normalizarBuscaPainel(texto).includes(normalizarBuscaPainel(busca))});d.innerHTML=itens.length?itens.map(x=>'<article class="linha-preco ingrediente-item"><strong>'+escapar(nomePizzaPainel(x.nome))+'</strong><label class="ingredientes-campo">Descrição<textarea rows="3" maxlength="500" data-ingredientes-pizza="'+encodeURIComponent(x.nome)+'">'+escapar(x.ingredientes||"")+'</textarea></label><button class="btn primario" data-salvar-ingredientes="'+encodeURIComponent(x.nome)+'">Salvar</button></article>').join(""):'<div class="vazio">Nenhum produto encontrado.</div>'}
...
document.addEventListener("input",e=>{if(e.target.id==="buscarAdicionais")renderAdicionais();const nome=e.target.dataset.adicionalNome,preco=e.target.dataset.adicionalPreco,indice=Number(e.target.dataset.adicionalIndice);if(!nome&&!preco)return;const produto=(estado.adicionais||[]).find(item=>item.nome===decodeURIComponent(nome||preco));if(!produto||!produto.adicionais[indice])return;if(nome)produto.adicionais[indice].nome=e.target.value;if(preco)produto.adicionais[indice].preco=e.target.value});
document.addEventListener("click",async e=>{const aba=e.target.closest(".aba-adicional");if(aba){estado.tipoAdicional=aba.dataset.tipoAdicional;document.querySelectorAll(".aba-adicional").forEach(x=>x.classList.toggle("ativa",x===aba));return renderAdicionais()}const adicionar=e.target.closest("[data-adicionar-adicional]"),remover=e.target.closest("[data-remover-adicional]"),salvar=e.target.closest("#salvarAdicionais");if(adicionar){const produto=(estado.adicionais||[]).find(item=>item.nome===decodeURIComponent(adicionar.dataset.adicionarAdicional));if(produto){produto.adicionais.push({nome:"",preco:""});renderAdicionais()}return}if(remover){const produto=(estado.adicionais||[]).find(item=>item.nome===decodeURIComponent(remover.dataset.removerAdicional));if(produto){produto.adicionais.splice(Number(remover.dataset.adicionalIndice),1);renderAdicionais()}return}if(!salvar)return;try{salvar.disabled=true;salvar.textContent="Salvando...";const adicionais=Object.fromEntries((estado.adicionais||[]).map(item=>[item.nome,item.adicionais||[]]));const resposta=await api("/api/painel/adicionais",{method:"PUT",body:JSON.stringify({adicionais})});estado.adicionais=(estado.adicionais||[]).map(item=>({...item,adicionais:resposta.adicionais?.[item.nome]||[]}));renderAdicionais();toast("Adicionais salvos.")}catch(erro){toast(erro.message)}finally{salvar.disabled=false;salvar.textContent="Salvar alterações"}});
